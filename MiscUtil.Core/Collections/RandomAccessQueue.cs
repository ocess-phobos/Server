...
namespace MiscUtil.Core.Collections
{
	...
	public sealed class RandomAccessQueue<T> : ICollection<T>, ICollection, IEnumerable<T>, ICloneable
	{
		...
		{
			int newCapacity = Math.Max(this.Count, DefaultCapacity);
		...
		public T RemoveAt(int index)
		{
			...
			if (this.start+index >= this.Capacity)
			{
				...
				Array.Copy(this.buffer, this.start+index-this.Capacity+1, 
                           this.buffer, this.start+index-this.Capacity, 
                           this.count-index-1);
                this.buffer[(this.start + this.count - 1) - this.Capacity] = default(T);
			}
			else
			{
		...
		public int BinarySearch (T obj)
		{
			if (obj==null)
			{
				if (this.count==0 || this.buffer[this.start]!=null)
				{
					return ~0;
				}
				else
        ...
        {
            return this.BinarySearch(obj, new ComparisonComparer<T>(comparison));
        }        
		...
		{
			return new RandomAccessQueue<T>(this.buffer, this.count, this.start);
		}
		#endregion	
