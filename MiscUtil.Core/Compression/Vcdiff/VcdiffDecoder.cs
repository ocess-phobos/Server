...
namespace MiscUtil.Core.Compression.Vcdiff
{
	...
	public sealed class VcdiffDecoder
	{
		...
		void ReadHeader()
		{
			byte[] header = IOHelper.CheckedReadBytes(this.delta, 4);
			if (header[0] != 0xd6 ||
				header[1] != 0xc3 ||
				header[2] != 0xc4)
			{
			...
			byte headerIndicator = IOHelper.CheckedReadByte(this.delta);
			...
			{
				int appHeaderLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
		...
		void ReadCodeTable()
		{
			...
			int compressedTableLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta)-2;
			int nearSize = IOHelper.CheckedReadByte(this.delta);
			int sameSize = IOHelper.CheckedReadByte(this.delta);
			byte[] compressedTableData = IOHelper.CheckedReadBytes(this.delta, compressedTableLength);
		...
		bool DecodeWindow ()
		{
			int windowIndicator = this.delta.ReadByte();
			// Have we finished?
			...
			if (sourceStream != null)
			{
				...
				int sourcePosition = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
			...
			int targetLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
			...
			byte deltaIndicator = IOHelper.CheckedReadByte(this.delta);
			if (deltaIndicator != 0)
			{
				throw new VcdiffFormatException("VcdiffDecoder is unable to handle compressed delta sections.");
			}

			int addRunDataLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
			int instructionsLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
			int addressesLength = IOHelper.ReadBigEndian7BitEncodedInt(this.delta);
			...
			{
				byte[] checksumBytes = IOHelper.CheckedReadBytes(this.delta, 4);
			...
			byte[] addRunData = IOHelper.CheckedReadBytes(this.delta, addRunDataLength);
			byte[] instructions = IOHelper.CheckedReadBytes(this.delta, instructionsLength);
			byte[] addresses = IOHelper.CheckedReadBytes(this.delta, addressesLength);
				...
				for (int i=0; i < 2; i++)
				{
					...
					switch (instruction.Type)
					{
						...
						case InstructionType.Copy:
							int addr = this.cache.DecodeAddress((int)targetDataStream.Position+sourceLength, instruction.Mode);							
