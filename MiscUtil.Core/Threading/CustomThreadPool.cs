	...
	public class CustomThreadPool
	{
		...
		public ThreadPriority WorkerThreadPriority
		{
			...
			set
			{
				lock (this.stateLock)
				{
					this.workerThreadPriority = value;
				}
			}
		}
			...
			lock (this.stateLock)
			{
				lock (this.queueLock)
				{
					...
					else
					{
						...
						if (((ThreadPoolWorkItem)this.queue[this.queue.Count-1]).Priority >= workItem.Priority)
						{
							this.queue.Enqueue(workItem);
						}
						else
						{
							...
							int position = this.queue.BinarySearch(workItem, PriorityComparer.Instance);
		...
		public bool CancelWorkItem(object id)
		{
			...
			lock (this.queueLock)
			{
				...
				{
					ThreadPoolWorkItem item = (ThreadPoolWorkItem) this.queue[i];
		...
		void StartWorkerThread()
		{
			...
			Thread thread = new Thread(new ThreadStart(this.WorkerThreadLoop));
			thread.Name = this.Name+" thread "+this.threadCounter;
		...
		void WorkerThreadLoop()
		{
			...
			try
			{
				...
				while (true)
				{
					...
					int waitPeriod = this.CalculateWaitPeriod (lastJob);
		...
		int CalculateWaitPeriod (DateTime lastJob)
		{
			lock (this.stateLock)
			{
				int waitPeriod = this.IdlePeriod;
		...
		void ExecuteWorkItem(ThreadPoolWorkItem job)
		{
			...
			{
				this.workingThreads++;
				Thread.CurrentThread.Priority = this.workerThreadPriority;
				Thread.CurrentThread.IsBackground = this.workerThreadsAreBackground;
			}
			try
			{
				bool cancel;
				this.OnBeforeWorkItem(job, out cancel);
				if (cancel)
				{
					return;
				}
