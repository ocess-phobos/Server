    ...
    public sealed class ReverseLineReader : IEnumerable<string>
    {
        ...
        private IEnumerator<string> GetEnumeratorImpl(Stream stream)
        {
            using (stream)
            {
                ...
                while (position > 0)
                {
                    int bytesToRead = Math.Min(position > int.MaxValue ? this.bufferSize : (int)position, this.bufferSize);
                    ...
                    int charsRead = this.encoding.GetChars(buffer, firstCharPosition, bytesToRead - firstCharPosition, charBuffer, 0);
